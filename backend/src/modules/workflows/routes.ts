...
export async function workflowRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/workflows/:id/triggers', async (request, reply) => {
    const { id } = request.params as { id: string };
    const body = request.body as { trigger_type: string; trigger_config: Record<string, unknown> };
  ...
  fastify.post('/capabilities/grant', async (request, reply) => {
    const body = request.body as { user_id: string; capability_name: string };
    const userId = (request as any).user?.id;
  ...
  fastify.post('/commands/execute', async (request, reply) => {
    const body = request.body as { command_name: string; args: Record<string, unknown> };
    const userId = (request as any).user?.id;
