...
export function ProjectPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'chat' | 'files' | 'meetings' | 'settings'>('overview');
  ...
  return (
    <WorkspaceLayout
      currentOrganizationId={currentOrganizationId}
      currentWorkspaceId={currentWorkspaceId}
      rightPanelContent={activeTab === 'overview' || activeTab === 'settings' ? rightPanelContent : undefined}
      rightPanelTitle="Context"
    >
      <Topbar
        title={project.name}
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex border-b border-glass-border">
          {[
            ...
            { id: 'meetings', label: 'Meetings', icon: <Video className="h-4 w-4" /> },
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-2 border-b-2 px-6 py-3 text-sm font-medium transition-all duration-200 -mb-[2px]',
                  isActive
        ...
        {activeTab === 'overview' && (
          <ProjectOverview projectId={projectId || ''} onTabChange={(tab) => setActiveTab(tab as any)} />
        )}
              ...
              <div className="rounded-2xl border border-glass-border bg-card/70 p-6 shadow-soft-md">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <div className="rounded-lg border border-glass-border bg-background/50 px-4 py-3">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-medium text-foreground">Private project</p>
                          <p className="text-xs text-muted-foreground">
                            Invite-only. Only active project members can view it.
                          </p>
                        </div>
                        <span className="rounded-full bg-primary/15 px-3 py-1 text-xs font-medium text-primary">
                          Private
